...
import { emptySLARowForm, hoursToSeconds, SEVERITY_LEVELS, type SLARowForm } from '@/lib/slaTargets'
import type { ServiceSLAResponse } from '@/types/api'

function toForm(sla?: ServiceSLAResponse): SLARowForm {
  return {
    mttd: sla?.mttd_target_seconds ? String(Math.round(Number(sla.mttd_target_seconds) / 3600)) : '',
    mttm: sla?.mttm_target_seconds ? String(Math.round(Number(sla.mttm_target_seconds) / 3600)) : '',
    mttr: sla?.mttr_target_seconds ? String(Math.round(Number(sla.mttr_target_seconds) / 3600)) : '',
    rtpc: sla?.rtpc_target_seconds ? String(Math.round(Number(sla.rtpc_target_seconds) / 3600)) : '',
  }
}

/** One target-hours column header, an acronym plus an info icon whose
  ...
  return (
    <th className="py-2 pr-3">
      <span className="inline-flex items-center gap-1">
        {label} target (hrs)
        <InfoTooltip text={definition} side="bottom" />
      </span>
    </th>
  )
}
  ...
  return (
    <div className="flex flex-col gap-2 rounded-md border border-border p-3">
      <p className="text-xs text-muted-foreground">
        Target response times per severity level, in whole hours. A blank field means no SLA is set for that metric —
        no breach indicator shows on a SEV unless at least one of its affected services configures a target here.
        When a SEV has multiple affected services, the strictest configured target across all of them applies.
      </p>
                ...
                <tr key={level} className="border-b border-border align-top last:border-0">
                  ...
                  <td className="py-2 pr-3">
                    <Input
                      type="number"
                      min={0}
                      aria-label={`MTTD target hours for SEV-${level}`}
                      value={form.mttd}
                  ...
                  <td className="py-2 pr-3">
                    <Input
                      type="number"
                      min={0}
                      aria-label={`MTTM target hours for SEV-${level}`}
                      value={form.mttm}
                  ...
                  <td className="py-2 pr-3">
                    <Input
                      type="number"
                      min={0}
                      aria-label={`MTTR target hours for SEV-${level}`}
                      value={form.mttr}
                  ...
                  <td className="py-2 pr-3">
                    <Input
                      type="number"
                      min={0}
                      aria-label={`RTPC target hours for SEV-${level}`}
                      value={form.rtpc}
