...
const CHECKOUT: ServiceResponse = { id: 'checkout', name: 'Checkout', active: true, created_at: 'now', updated_at: 'now' }
...
describe('LevelingCriteriaPanel', () => {
  ...
  it('renders nothing when no services are selected', () => {
    const { container } = renderWithProviders(
      <LevelingCriteriaPanel severityLevel={1} serviceIds={[]} services={[]} />,
    )
  ...
  it('shows guidance text per service when populated', async () => {
    ...
    renderWithProviders(<LevelingCriteriaPanel severityLevel={1} serviceIds={['checkout']} services={[CHECKOUT]} />)
  ...
  it('falls back to the raw service id when the service is not in the registry', async () => {
    vi.mocked(fetch).mockImplementation((input) => {
      const url = String(input)
      if (url.startsWith('/v1/config/leveling-criteria')) {
        return Promise.resolve(
          jsonResponse({
            criteria: [
              { service_id: 'checkout', severity_level: 1, criteria: 'guidance', created_at: 'now', updated_at: 'now' },
            ],
          }),
        )
      }
    ...
    renderWithProviders(<LevelingCriteriaPanel severityLevel={1} serviceIds={['checkout']} services={[]} />)
  ...
  it('shows a quiet empty-state note when nothing is configured', async () => {
    ...
    renderWithProviders(<LevelingCriteriaPanel severityLevel={2} serviceIds={['checkout']} services={[CHECKOUT]} />)
